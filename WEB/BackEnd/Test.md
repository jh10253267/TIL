...
예를 들어 어떤 테스트 대상이 있다고 하자. PostService는 게시글과 관련된 작업을 수행하는 클래스이다. PostService의 로직만 검증하고싶은데 내부적으로 PostRepository라는 클래스의 기능을 사용하여 동작한다.

그렇다면 PostService를 테스트할 때 PostRepository를 목 객체로 선언해주면 된다. 그 방법은 @Mock 어노테이션을 PostRepository위에 붙여주는 것이다.

비슷한 어노테이션으론 @MockBean이 있는데 둘의 차이는 @Mock은 단위 테스트에서 사용되고 Mock객체를 직접 생성하여 사용하기 때문에 스프링 컨텍스트와 관련이 없고 @MockBean은 Mock객체를 생성하고 스프링 컨텍스트에 등록한다는 점이다.
이렇게 생성한 Mock 객체는 아무런 로직을 수행하지 않는다. 마치 인터페이스처럼 입출력에 대한 정보만 가지고있다.

그래서 테스트하려면 해당 객체의 행동을 정의해주어야한다.

이는 when()메소드를 이용해서 정의해준다.

목 객체가 호출되었을 때의 로직을 넣어주면 된다.
