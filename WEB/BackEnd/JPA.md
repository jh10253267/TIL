...
이렇게 되면 엔티티와 DB간의 관계가 명확하지 않아진다.
...
2. 다중성이 적은 방향을 선택하는 것이 바람직하다. Post와 Comment가 일대다로 되어있다면 Comment가 변경될 때 Post도 변경된다. 반면 다대일로 되어있다면 Comment가 변경될 때 Comment에만 변경이 일어난다.

이런 이유로 @OneToMany를 사용하여 단방향 연관관계를 맺는 것 보다 @ManyToOne을 사용하여 단방향 연관관계를 맺는 것이 좋다.

아무튼 이러한 ORM의 패러다임을 실현한게 바로 JPA이다.
...
간단히 검색기능을 구현하고 싶다면 이렇게 작성할 수 있다.
...
자바 코드를 이용하기 때문에 런타임 시점이 아니라 컴파일 시점에 오류를 찾아낼 수 있기 때문에 타입의 안정성을 확보할 수 있다는 장점이있다. 이 이점은 생각보다 크다. 예를 들어 JPQL로 작성한 쿼리에 오류가 있는데 해당 경로가 호출되는 빈도가 굉장히 낮다고 하면 오류가 있다는 것을 알기까지 시간이 얼마나 걸릴지 모른다. 그러나 Querydsl을 사용하면 이런 오류를 빠르게 인지할 수 있다.
