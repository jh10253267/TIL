...
자바의 객체로 본다면 post에서 comment 방향으로 접근하지만 comment에서 Post로는 접근할 수 없다.
...
2. 다중성이 적은 방향을 선택하는 것이 바람직하다. Post와 Comment가 일대다로 되어있다면 Comment가 변경될 때 Post도 변경된다. 반면 다대일로 되어있다면 Comment가 변경될 때 Comment에만 변경이 일어난다. 자신의 필드를 관리하는 번거로움 존재.
...
만약 상위 튜플을 삭제하려면 하위 튜플들을 모두 삭제시킨 후 상위 튜플을 삭제하면 되는 것이다.
```
### 주의점
...
JPA를 사용하다 보면 조금 이상한 부분을 발견할 수 있는데 바로 애플리케이션에서 자바 엔티티의 값을 변경했을 뿐인데 데이터베이스 업데이트 쿼리가 발생하고 실제 DB에도 반영이 된다는 점이다. 사실 튜플 수정을 위한 메소드인 repository.save()가 존재한다. 그러나 이 메소드를 사용하지 않았을 때 repository를 거치지 않았으니 DB에 반영되지 않고 객체에만 변경이 생길 것이라 예상했는데 말이다.
...
상위 엔티티와 하위 엔티티의 연관관계를 사우이 엔티티에서 관리했을 때 중요한 것은 상위 엔티티의 상태가 변경되었을 때 하위 엔티티 객체들 역시 같이 영향을 받는다는 것이다.
