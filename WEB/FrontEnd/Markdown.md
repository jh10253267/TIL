...
주로 사용되는 곳이 웹이기 때문에 마크업은 HTML로 변환되는데  모든 HTML 마크업을 대체할 수 없다. 그래서 HTML코드를 섞어서 작성하는 방법을 사용할 수도 있다.
...
이들은 각각 h1~h6로 대치된다. 즉 #의 갯수가 중요도를 결정한다.

제목의 내용은 다음과 같이 작성하면 된다.
...
그리고 리스트 안에 리스트를 넣는 경우도 많은데 사용하는 방법은 리스트의 안에 4칸 띄어쓰기를 하고 다시 리스트를 작성하면 된다.
...
[텍스트](링크)
```
그리고 링크를 클릭했을 때 현재 창에서 이동할 수도 있고 새 탭에서 열릴 수도 있다. 
```
